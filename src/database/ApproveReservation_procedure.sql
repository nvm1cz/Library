...
CREATE PROCEDURE ApproveReservation(IN reservation_id INT)
BEGIN
    DECLARE v_book_id INT;
    DECLARE v_account_id INT;
    DECLARE v_available_copies INT;

    -- Get book ID and account ID from reservation
    SELECT BookID, AccountID INTO v_book_id, v_account_id
    FROM Reservation
    ...
    IF v_available_copies > 0 THEN
        ...
        WHERE BookID = v_book_id
          AND Status = 'Pending'
          AND ReservationDate > (
              SELECT ReservationDate FROM Reservation WHERE ReservationID = reservation_id
          );
